...
//          foreach($aRow as $t)echo "$t\n\r";
          ?>
          <option value="<?= $type_id ?>" <?php if($type_id==$eType) echo "selected" ?>><?= $type_name; ?></option>
          <?php
         }
         ?>
         </select>
...
<td align="center" width="50%"><strong><?= gettext("Display Events in Year") ?></strong><br>
       <select name="WhichYear" onchange="javascript:this.form.submit()" >
        ...
        {
          ?>
          <option value="<?= $Yr[$r] ?>" <?php if($Yr[$r]==$EventYear) echo "selected" ?>><?= $Yr[$r]; ?></option>
          <?php
         }
         ?>
         </select>
       ...
       <caption>
         ...
         <tr class="<?= $sRowClass; ?>">
           ...
           <?= ($aEventDesc[$row] == '' ? "&nbsp;":$aEventDesc[$row]); ?>
             <?= ($aEventText[$row] != '' ? "&nbsp;&nbsp;&nbsp;<a href=\"javascript:popUp('GetText.php?EID=".$aEventID[$row]."')\"><strong>Sermon Text</strong></a>":""); ?></span>
           </td>
           <td>
