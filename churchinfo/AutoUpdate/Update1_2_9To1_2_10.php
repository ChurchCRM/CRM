...
$sSQL = "INSERT INTO `config_cfg` VALUES (2001, 'sMRBS_eable_periods', '', 'boolean', '0', 'Use \"clock\" based intervals (FALSE) or user defined periods (TRUE).  If user-defined periods are used then sMRBS_resolution, sMRBS_morningstarts, sMRBS_eveningends, sMRBS_morningstart_min and sMRBS_eveningends_min are ignored.', 'General')";
RunQuery($sSQL, FALSE); 
// False means do not stop on error


$sSQL = "INSERT INTO `config_cfg` VALUES (2002, 'sMRBS_resolution', '1800', 'number', '1800', 'Resolution - what blocks can be booked, in seconds.', 'General')";
RunQuery($sSQL, FALSE);

$sSQL = "INSERT INTO `config_cfg` VALUES (2003, 'sMRBS_morningstarts', '7', 'number', '7', 'Start of day, integer hours only, 0-23.\r\nsMRBS_morningstarts must be < sMRBS_eveningends. See also sMRBS_eveningends.', 'General')";
RunQuery($sSQL, FALSE);

$sSQL = "INSERT INTO `config_cfg` VALUES (2004, 'sMRBS_eveningends', '19', 'number', '19', 'endof day, integer hours only, 0-23.\r\nsMRBS_eveningends must be > sMRBS_eveningends. See also sMRBS_morningstarts.', 'General')";
RunQuery($sSQL, FALSE);

$sSQL = "INSERT INTO `config_cfg` VALUES (2005, 'sMRBS_morningstart_min', '0', 'number', '0', 'Minutes to add to sMRBS_morningstarts to get to the real start of the day. Be sure to consider the value of sMRBS_eveningends_min if you change this, so that you do not cause a day to finish before the start of the last period. ', 'General')";
RunQuery($sSQL, FALSE);

$sSQL = "INSERT INTO `config_cfg` VALUES (2006, 'sMRBS_eveningends_min', '0', 'number', '0', 'Minutes to add to sMRBS_eveningends hours to get the real end of the day.', 'General')";
RunQuery($sSQL, FALSE);

$sSQL = "INSERT INTO `config_cfg` VALUES (2007, 'sMRBS_weekstarts', '0', 'number', '0', 'Start of week: 0 for Sunday, 1 for Monday, 2 for Tuesday etc.', 'General')";
RunQuery($sSQL, FALSE);

$sSQL = "INSERT INTO `config_cfg` VALUES (2008, 'sMRBS_dateformat', '0', 'number', '0', 'Trailer date format: 0 to show dates as \"Jul 10\", 1 for \"10 Jul\"', 'General')";
RunQuery($sSQL, FALSE);

$sSQL = "INSERT INTO `config_cfg` VALUES (2009, 'sMRBS_24hrs_format', '1', 'number', '1', 'Time format in pages. 0 to show dates in 12 hour format, 1 to show them in 24 hour format', 'General')";
RunQuery($sSQL, FALSE);

$sSQL = "INSERT INTO `config_cfg` VALUES (2010, 'sMRBS_default_rpt_days', '60', 'number', '60', 'Default report span in days', 'General')";
RunQuery($sSQL, FALSE);

$sSQL = "INSERT INTO `config_cfg` VALUES (2011, 'sMRBS_search_count', '20', 'number', '20', 'Results per page for searching', 'General')";
RunQuery($sSQL, FALSE);

$sSQL = "INSERT INTO `config_cfg` VALUES (2012, 'sMRBS_refresh_rate', '0', 'number', '0', 'Page refresh time (in seconds). Set to 0 to disable', 'General')";
RunQuery($sSQL, FALSE);

$sSQL = "INSERT INTO `config_cfg` VALUES (2013, 'sMRBS_area_list_fmt', 'list', 'text', 'list', 'should areas be shown as a list or a drop-down select box? (list / select)', 'General')";
RunQuery($sSQL, FALSE);

$sSQL = "INSERT INTO `config_cfg` VALUES (2014, 'sMRBS_mon_v_entries_dtl', 'both', 'text', 'both', 'Entries in monthly view can be shown as start/end slot, brief description or\r\nboth. Set to \"description\" for brief description, \"slot\" for time slot and \"both\" for both. Default is \"both\", but 6 entries per day are shown instead of 12.\r\n', 'General')";
RunQuery($sSQL, FALSE);


$sSQL = "INSERT INTO `userconfig_ucfg` VALUES (0, 8, 'bEditMRBSBooking', '', 'boolean', 'Reserve resources in MRBS system, modify reservations and delete self reservations', 'TRUE')";
RunQuery($sSQL, FALSE);

$sSQL = "INSERT INTO `userconfig_ucfg` VALUES (0, 9, 'bAddMRBSResource', '', 'boolean', 'Add, modify and delete resources in MRBS, modify other user''s reservations.', 'TRUE')";
RunQuery($sSQL, FALSE);


$sSQL = "DROP TABLE IF EXISTS `mrbs_area`";
RunQuery($sSQL, FALSE);

$sSQL = "CREATE TABLE `mrbs_area` (
...
$sSQL = "DROP TABLE IF EXISTS `mrbs_entry`";
RunQuery($sSQL, FALSE);

$sSQL = "CREATE TABLE `mrbs_entry` (
  `id` int(11) NOT NULL auto_increment,
...
$sSQL = "DROP TABLE IF EXISTS `mrbs_repeat`";
RunQuery($sSQL, FALSE);

$sSQL = "CREATE TABLE `mrbs_repeat` (
  `id` int(11) NOT NULL auto_increment,
  `start_time` int(11) NOT NULL default '0',
  `end_time` int(11) NOT NULL default '0',
...
$sSQL = "DROP TABLE IF EXISTS `mrbs_room`";
RunQuery($sSQL, FALSE);

$sSQL = "CREATE TABLE `mrbs_room` (
  `id` int(11) NOT NULL auto_increment,
...
$sSQL = "INSERT INTO `menuconfig_mcf` VALUES (61, 'booking', 'root', 1, 'Booking', '', 'Resources Reservation', 'bAll', NULL, 0, 0, NULL, 1, 10)";
RunQuery($sSQL, FALSE);

$sSQL = "INSERT INTO `menuconfig_mcf` VALUES (62, 'overview', 'booking', 0, 'Reservation', 'mrbs/index.php', '', 'bAll', NULL, 0, 0, NULL, 1, 1)";
RunQuery($sSQL, FALSE);

$sSQL = "INSERT INTO `menuconfig_mcf` VALUES (63, 'bookingreport', 'booking', 0, 'Reports', 'mrbs/report.php', '', 'bAll', NULL, 0, 0, NULL, 1, 2)";
RunQuery($sSQL, FALSE);
