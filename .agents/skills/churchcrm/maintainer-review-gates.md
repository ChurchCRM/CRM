...
7. Repo process — no linked issue; lint/build clearly failing; **title or description that does not match the current diff** (describes other work, or omits the main change).
...
- Title/body wording that is only slightly stale — ask the author (or update it on maintainer PRs) instead of blocking.

## Review shape

1. Thank the contributor. Say what the PR does in one short paragraph — after reading the file list, not only the title.
2. Check title + body against `git diff --name-status origin/master...HEAD`. If they drifted, say so.
3. Hard blocks first. These are Request changes.
4. Follow-ups second.
5. If there are no hard blocks, GitHub event is Comment. A human decides approve/merge later.
6. Never approve. Never merge. Never merge or close an issue unless the maintainer answers yes to a direct question.
