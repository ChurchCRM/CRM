...
- **Orphaned profile images after delete** – Deleting a family (with or without members) or a person now removes the uploaded image from disk via a Propel `preDelete` hook on both models, keeping `Images/Family/` and `Images/Person/` clean on any delete path (#1697)
