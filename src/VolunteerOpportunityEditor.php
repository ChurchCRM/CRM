                    ...
                    <div class="card-body" style="overflow: visible;">
                        ...
                        for ($row = 1; $row <= $numRows; $row++) {
                            if (array_key_exists($row, $aNameFields)) {
                                ...
                                // False positive: $aIDFields[$row] is an ORM integer ID, not user input. Both tainted-sql-string and echoed-request rules mis-fire here.
                                echo '<a href="VolunteerOpportunityEditor.php?act=delete&amp;Opp=' . $aIDFields[$row] . '" class="dropdown-item text-danger"><i class="fa-solid fa-trash me-2"></i>' . gettext('Delete') . '</a>'; // nosemgrep: php.lang.security.injection.tainted-sql-string.tainted-sql-string, php.lang.security.injection.echoed-request.echoed-request
                                echo '</div>';
                                echo '</div>';
                                echo '</td>';
                                echo '</tr>';
