...
if ($output === 'pdf') {
    ...
    foreach ($rsReport as $row) {
        ...
        if ($fam_ID != $currentFamilyID && $currentFamilyID != -1) {
            ...
            $totalAmountStr = ChurchInfoReport::convertToLatin1(CurrencyFormatter::format((float) $totalAmount));
            $pdf->SetFont('Courier', '', 9);
            $pdf->Cell(25, $summaryIntervalY, $totalAmountStr, 0, 1, 'R');
            $pdf->SetFont('Times', 'B', 10);
            $pdf->Cell(95, $summaryIntervalY, ' ');
            $pdf->Cell(50, $summaryIntervalY, 'Goods and Services Rendered:');
            $totalAmountStr = ChurchInfoReport::convertToLatin1(CurrencyFormatter::format((float) $totalNonDeductible));
            $pdf->SetFont('Courier', '', 9);
            $pdf->Cell(25, $summaryIntervalY, $totalAmountStr, 0, 1, 'R');
            $pdf->SetFont('Times', 'B', 10);
            $pdf->Cell(95, $summaryIntervalY, ' ');
            $pdf->Cell(50, $summaryIntervalY, 'Tax-Deductible Contribution:');
            $totalAmountStr = ChurchInfoReport::convertToLatin1(CurrencyFormatter::format((float) ($totalAmount - $totalNonDeductible)));
            $pdf->SetFont('Courier', '', 9);
            $pdf->Cell(25, $summaryIntervalY, $totalAmountStr, 0, 1, 'R');
            $curY = $pdf->GetY();
