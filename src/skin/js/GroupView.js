$(document).ready(function () {
  ...
  $(".personSearch").on("select2:select", function (e) {
  		var dataTable = window.CRM.DataTableAPIGroupView;
      window.CRM.groups.promptSelection({Type:window.CRM.groups.selectTypes.Role,GroupID:window.CRM.currentGroup},function(selection){
        window.CRM.groups.addPerson(window.CRM.currentGroup, e.params.data.objid,selection.RoleID).done(function (data) {
          ...
          window.CRM.DataTableAPIGroupView.ajax.reload();/* we reload the data no need to add the person inside the dataTable */          
        });
      });
  });

  $("#deleteSelectedRows").click(function () {
    var deletedRows = window.CRM.DataTableAPIGroupView.rows('.selected').data()
    bootbox.confirm({
        ...
        if (result)
        {
          $.each(deletedRows, function (index, value) {
            window.CRM.groups.removePerson(window.CRM.currentGroup,value.PersonId).done(
              function(){
                window.CRM.DataTableAPIGroupView.row(function (idx, data, node) {
                  if (data.PersonId == value.PersonId) {
                    return true;
                  }
                }).remove();
                window.CRM.DataTableAPIGroupView.rows().invalidate().draw(true);
            });
          });
        }
      }
  ...
  $("#addSelectedToCart").click(function () {
    if (window.CRM.DataTableAPIGroupView.rows('.selected').length > 0)
    {
      var selectedPersons = {
        "Persons" : $.map(window.CRM.DataTableAPIGroupView.rows('.selected').data(), function(val,i){
                      return val.PersonId;
                    })
      };
  ...
  $("#addSelectedToGroup").click(function () {
    window.CRM.groups.promptSelection({Type:window.CRM.groups.selectTypes.Group|window.CRM.groups.selectTypes.Role}, function(data){
      selectedRows = window.CRM.DataTableAPIGroupView.rows('.selected').data()
...
function initDataTable() {
  ...
  window.CRM.DataTableAPIGroupView = $("#membersTable").DataTable(DataTableOpts);
  ...
  $(document).on('click', '.groupRow', function () {
    ...
    var selectedRows = window.CRM.DataTableAPIGroupView.rows('.selected').data().length;
