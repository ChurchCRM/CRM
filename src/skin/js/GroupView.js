function initializeGroupView() {
  ...
  $("#deleteSelectedRows").on("click", function () {
    ...
    bootbox.confirm({
      message:
        i18next.t("Are you sure you want to remove the selected group members?") + " (" + deletedRows.length + ") ",
      buttons: {
        confirm: {
...
function initDataTable() {
  var DataTableOpts = {
    ...
    columns: [
      {
        ...
        render: function (data, type, full, meta) {
          ...
          return (
            '<a target="_top" href="PersonView.php?PersonID=' + full.PersonId + '">' + escapedName + "</a>" + photoIcon
          );
        },
      },
      {
    ...
    drawCallback: function (settings) {
      $("#iTotalMembers").text(settings.aoData.length);
    },
