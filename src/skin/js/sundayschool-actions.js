...
(function () {
  ...
  function createModal(title, bodyHtml) {
    ...
    wrapper.innerHTML =
      ...
      '<button type="button" class="btn btn-ghost-secondary" data-bs-dismiss="modal">' + i18next.t("Cancel") + "</button>" +
      '<button type="button" class="btn btn-primary" id="ssModalConfirmBtn" disabled>' + i18next.t("Save") + "</button>" +
      "</div></div></div>";
  ...
  function showGroupAndRoleModal(title, callback) {
    var body =
      '<div class="mb-3"><label class="form-label">' + i18next.t("Group") + "</label>" +
      '<select id="ss-group-select"></select></div>' +
      '<div class="mb-3 d-none" id="ss-role-wrapper"><label class="form-label">' + i18next.t("Role") + "</label>" +
    ...
    window.CRM.groups.get().done(function (groups) {
      ...
      populateSelect(groupEl, groups.map(function (g) {
        return { value: String(g.Id), text: g.Name };
      }));

      result.el.addEventListener("shown.bs.modal", function () {
        ...
        new window.TomSelect(groupEl, {
          ...
          onChange: function (value) {
            ...
            window.CRM.groups.getRoles(value).done(function (roles) {
              if (roles.length === 0) { selectedRoleId = null; result.confirm.disabled = false; return; }
              if (roles.length === 1) { selectedRoleId = String(roles[0].OptionId); result.confirm.disabled = false; return; }
              populateSelect(roleEl, roles.map(function (r) {
              ...
              new window.TomSelect(roleEl, {
                onChange: function (v) { selectedRoleId = v || null; },
              });
    ...
    window.CRM.onLocalesReady(function () {
      ...
      $(document).on("click", ".ss-copy-role", function (e) {
        ...
        showGroupAndRoleModal(i18next.t("Copy Members to Group"), function (data) {
          ...
          if (ids.length > 0) {
            window.CRM.notify(
              i18next.t("Copied") + " " + ids.length + " " + i18next.t("members"),
      ...
      $(document).on("click", ".ss-move-role", function (e) {
        ...
        bootbox.confirm({
          ...
          message: i18next.t("Are you sure you want to move") + " <strong>" +
            window.CRM.escapeHtml(label) + "</strong> " + i18next.t("to another group?"),
          buttons: {
