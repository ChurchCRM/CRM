...
abstract class Task implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\ChurchCRM\\Map\\TaskTableMap';
    ...
    /**
     * Initializes internal state of ChurchCRM\Base\Task object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\ChurchCRM\Task The current object (for fluent API support)
     */
    ...
    public function setType($v)
    {
        ...
        if ($this->type !== $v) {
            $this->type = $v;
            $this->modifiedColumns[TaskTableMap::COL_TYPE] = true;
        }
    ...
    public function setactive($v)
    {
        ...
        if ($this->active !== $v) {
            $this->active = $v;
            $this->modifiedColumns[TaskTableMap::COL_ACTIVE] = true;
        }
    ...
    public function setDesc($v)
    {
        ...
        if ($this->desc !== $v) {
            $this->desc = $v;
            $this->modifiedColumns[TaskTableMap::COL_DESC] = true;
        }
    ...
    public function setremoteId($v)
    {
        ...
        if ($this->remote_id !== $v) {
            $this->remote_id = $v;
            $this->modifiedColumns[TaskTableMap::COL_REMOTE_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : TaskTableMap::translateFieldName('Type', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : TaskTableMap::translateFieldName('active', TableMap::TYPE_PHPNAME, $indexType)];
            $this->active = (null !== $col) ? (boolean) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : TaskTableMap::translateFieldName('Desc', TableMap::TYPE_PHPNAME, $indexType)];
            $this->desc = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : TaskTableMap::translateFieldName('remoteId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 6; // 6 = TaskTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\ChurchCRM\\Task'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(TaskTableMap::COL_TYPE)) {
            $modifiedColumns[':p' . $index++]  = 'type';
        }
        if ($this->isColumnModified(TaskTableMap::COL_TITLE)) {
            $modifiedColumns[':p' . $index++]  = 'title';
        }
        if ($this->isColumnModified(TaskTableMap::COL_ACTIVE)) {
            $modifiedColumns[':p' . $index++]  = 'active';
        }
        if ($this->isColumnModified(TaskTableMap::COL_DESC)) {
            $modifiedColumns[':p' . $index++]  = 'desc';
        }
        if ($this->isColumnModified(TaskTableMap::COL_REMOTE_ID)) {
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\ChurchCRM\Task The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(TaskTableMap::COL_TYPE)) {
            $criteria->add(TaskTableMap::COL_TYPE, $this->type);
        }
        if ($this->isColumnModified(TaskTableMap::COL_TITLE)) {
            $criteria->add(TaskTableMap::COL_TITLE, $this->title);
        }
        if ($this->isColumnModified(TaskTableMap::COL_ACTIVE)) {
            $criteria->add(TaskTableMap::COL_ACTIVE, $this->active);
        }
        if ($this->isColumnModified(TaskTableMap::COL_DESC)) {
            $criteria->add(TaskTableMap::COL_DESC, $this->desc);
        }
        if ($this->isColumnModified(TaskTableMap::COL_REMOTE_ID)) {
            $criteria->add(TaskTableMap::COL_REMOTE_ID, $this->remote_id);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \ChurchCRM\Task (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \ChurchCRM\Task Clone of current object.
     * @throws PropelException
     */
