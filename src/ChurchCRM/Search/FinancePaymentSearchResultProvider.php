            ...
            if ($Payments->count() > 0) {
                ...
                foreach ($Payments as $Payment) {
                    ...
                    if ($Payment->getVirtualColumn('GroupAmount') >= $min && $Payment->getVirtualColumn('GroupAmount') <= $max) {
                        $searchResults[] = new SearchResult('finance-payment-' . $id, CurrencyFormatter::format((float) $Payment->getVirtualColumn('GroupAmount')) . ' Payment on Deposit ' . $Payment->getDepid(), $Payment->getVirtualColumn('uri'));
                    }
                }
            }
        } catch (\Exception $e) {
