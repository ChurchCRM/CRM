 ...
 *
 * Errors are `VolunteerException` — the shared Volunteer v2 error carrying its own
...
class VolunteerAssignmentService
{
    ...
    public function assign(
        ...
        if (!$inPool && !$selfSignup) {
            if (!$allowOutsidePool) {
                throw VolunteerException::conflict(gettext(
        ...
        if ($existing !== null && !in_array($existing->getStatus(), self::REUSABLE_STATUSES, true)) {
            ...
            throw VolunteerException::conflict(gettext('That person is already assigned to this position on this occurrence'));
        }
        ...
        try {
            ...
            throw VolunteerException::conflict(gettext('That assignment could not be saved'));
        }
    }
    ...
    public function respond(
        ...
        if (!in_array($response, [VolunteerAssignment::STATUS_ACCEPTED, VolunteerAssignment::STATUS_DECLINED], true)) {
            throw VolunteerException::invalid(gettext('A response must be accepted or declined'));
        }
        ...
        if (!$isSelf && !$isCoordinator) {
            throw VolunteerException::forbidden(gettext('Not authorized for this assignment'));
        }
        ...
        if ($channel === VolunteerResponse::CHANNEL_COORDINATOR && !$isCoordinator) {
            throw VolunteerException::forbidden(gettext('Only a coordinator may record a response on someone else\'s behalf'));
        }
    ...
    public function setStatus(
        ...
        return match ($status) {
            ...
            default => throw VolunteerException::invalid(gettext('That status cannot be set directly')),
        };
    }
    ...
    public function proposeSubstitute(
        ...
        if ((int) $assignment->getPersonId() !== (int) $actor->getId()) {
            throw VolunteerException::forbidden(gettext('Only the assigned volunteer may propose a substitute'));
        }

        if (!in_array($assignment->getStatus(), self::LIVE_STATUSES, true)) {
            throw VolunteerException::conflict(gettext('This assignment is no longer active'))
        ...
        if ($substitutePersonId === (int) $assignment->getPersonId()) {
            throw VolunteerException::invalid(gettext('You cannot propose yourself as your own substitute'));
        }
        ...
        $clash = VolunteerAssignmentQuery::create()
            ...
            ->findOne();
        if ($clash !== null) {
            throw VolunteerException::forbidden(gettext('That person already holds this position on this occurrence'));
        }
        ...
        $pending = VolunteerSwapQuery::create()
            ...
            ->findOne();
        if ($pending !== null) {
            throw VolunteerException::conflict(gettext('A substitute has already been proposed for this assignment'));
        }
    ...
    public function approveSwap(VolunteerSwap $swap, User $actor, ?string $comment = null): array
    {
        ...
        if ($existing !== null && in_array($existing->getStatus(), self::LIVE_STATUSES, true)) {
            throw VolunteerException::conflict(gettext('That person already holds this position on this occurrence'));
        }
    ...
    public function withdrawSwap(VolunteerSwap $swap, User $actor, ?string $comment = null): VolunteerSwap
    {
        if ((int) $swap->getProposedByPersonId() !== (int) $actor->getId()) {
            throw VolunteerException::forbidden(gettext('Only the volunteer who proposed a substitute may withdraw it'));
        }

        if ($swap->getStatus() !== VolunteerSwap::STATUS_PROPOSED) {
            throw VolunteerException::conflict(gettext('This substitution request has already been decided'))
    ...
    public function removeVolunteerFromMinistry(VolunteerMinistry $ministry, int $personId, User $actor): array
    {
        ...
        try {
            ...
            $removedFromPool = $this->ministries->isInPool($ministryId, $personId);
            if ($removedFromPool) {
    ...
    public function getEligiblePeople(VolunteerOccurrence $occurrence, VolunteerPosition $position, ?string $query = null): array
    {
        ...
        $poolIds = array_flip($this->ministries->getPoolPersonIds(
    ...
    private function assertNotHistorical(VolunteerAssignment $assignment): void
    {
        ...
        if ($end !== null && $end < DateTimeUtils::getToday()) {
            throw VolunteerException::conflict(gettext('This occurrence has already happened and its assignments can no longer be changed'))
                ->withExtra(['currentStatus' => $assignment->getStatus()]);
        }
    }
    ...
    private function assertOccurrenceAssignable(VolunteerOccurrence $occurrence): void
    {
        if ($occurrence->getStatus() === VolunteerOccurrence::STATUS_CANCELLED) {
            throw VolunteerException::conflict(gettext('This occurrence has been cancelled'));
        }

        $end = $this->occurrenceEnd($occurrence);
        if ($end !== null && $end < DateTimeUtils::getToday()) {
            throw VolunteerException::conflict(gettext('This occurrence has already happened'));
        }
    }
    ...
    private function assertPositionBelongsToSchedule(VolunteerPosition $position, VolunteerSchedule $schedule): void
    {
        if ((int) $position->getMinistryId() !== (int) $schedule->getMinistryId()) {
            throw VolunteerException::invalid(gettext('That position belongs to a different ministry'));
        }

        if ((int) $position->getTeamId() !== (int) $schedule->getTeamId()) {
            throw VolunteerException::invalid(gettext('That position belongs to a different team'));
        }
    }
    ...
    private function assertCapacityAvailable(VolunteerOccurrence $occurrence, VolunteerPosition $position): void
    {
        ...
        if ($requirement === null) {
            throw VolunteerException::conflict(gettext('That position is not being staffed on this occurrence'));
        }

        if ($requirement['openCount'] <= 0) {
            throw VolunteerException::conflict(gettext('That position is already fully staffed'));
        }
    }
    ...
    {
        return $this->ministries->isInPool((int) $schedule->getMinistryId(), $personId);
    }
    ...
    private function requireOccurrence(VolunteerAssignment $assignment): VolunteerOccurrence
    {
        $occurrence = VolunteerOccurrenceQuery::create()->findPk((int) $assignment->getOccurrenceId());
        if ($occurrence === null) {
            throw VolunteerException::notFound(gettext('Occurrence not found'));
        }
    ...
    private function requirePosition(int $positionId): VolunteerPosition
    {
        $position = VolunteerPositionQuery::create()->findPk($positionId);
        if ($position === null) {
            throw VolunteerException::notFound(gettext('Position not found'));
        }
    ...
    private function requireSwapAssignment(VolunteerSwap $swap): VolunteerAssignment
    {
        ...
        if ($assignment === null) {
            throw VolunteerException::notFound(gettext('Assignment not found'));
        }
    ...
    {
        if (!$this->authz->canManageAssignment($actor, $assignment)) {
            throw VolunteerException::forbidden(gettext('Not authorized for this substitution request'));
        }
