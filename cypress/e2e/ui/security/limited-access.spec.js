 ...
 * EditSelf=1 only, password "onlyMyFamily"). Confirms the exclusive-permission
 * model: login redirects to /external/limited-access, internal pages bounce back,
 * and internal API calls (even to her own family) return 403.
 */
describe("Self-only access — EditSelf account user reaches the verify page (amanda.black)", () => {
    ...
    it("Shows the self-service Verify Family Info link scoped to her own family", () => {
        loginAsEditSelf();
        cy.contains("Verify Family Info").should("exist");
        cy.contains("Log Out").should("exist");
        cy.contains("Verify Family Info").click();
        ...
        // Path A reaches the SAME verify page as Path B (the token link). The
        // privacy invariant must hold here too: notes are private and must never
        // be exposed on the verification page, only the verifiable info.
    ...
    it("Session-based internal API call to her OWN family is blocked (403)", () => {
        loginAsEditSelf();
        cy.request({
            method: "GET",
            url: "/api/family/20",
