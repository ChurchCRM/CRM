...
class TermExtractor {
    ...
    /**
     * Collect the gettext()/ngettext() calls out of the Member Portal's Twig
     * templates into a generated PHP file, so xgettext can read them.
     *
     * xgettext has no Twig parser, and in PHP mode everything outside
     * `<?php … ?>` is inert markup — so `{{ gettext('…') }}` in a .twig file is
     * invisible to it. Lifting the calls into a throwaway .php file is the same
     * trick php-inline-i18next-plugin.ts uses for inline i18next.t() calls in
     * .php views (#9723). The generated file is passed to xgettext with
     * --no-location, so nothing about it reaches messages.po but the strings.
     *
     * @returns {string|null} absolute path of the generated file, or null when
    ...
    extractTwigTerms() {
        ...
        try {
            twigFiles = execSync(
                'find . -iname "*.twig" | sort | grep -v ./vendor',
                { cwd: srcDir, encoding: 'utf8' }
            ).trim().split('\n').filter(f => f);
        } catch {
            // `grep` exits non-zero when nothing matched — no templates, no terms.
            return null;
        }
        ...
        // The quote character and the escaped body are copied verbatim, so the
        // msgid xgettext sees is byte-for-byte the one the template asks for.
        const GETTEXT_CALL = /\bgettext\(\s*(['"])((?:\\.|(?!\1)[^\\])*)\1\s*\)/g;
        const NGETTEXT_CALL =
            /\bngettext\(\s*(['"])((?:\\.|(?!\1)[^\\])*)\1\s*,\s*(['"])((?:\\.|(?!\3)[^\\])*)\3\s*,/g;

        const lines = ['<?php', '', '// Generated by locale/scripts/locale-build.js — do not edit or commit.', ''];
        let callCount = 0;
        for (const twigFile of twigFiles) {
            const source = fs.readFileSync(path.join(srcDir, twigFile), 'utf8');
            for (const match of source.matchAll(GETTEXT_CALL)) {
                lines.push(`gettext(${match[1]}${match[2]}${match[1]});`);
                callCount++;
            }
            for (const match of source.matchAll(NGETTEXT_CALL)) {
                lines.push(
                    `ngettext(${match[1]}${match[2]}${match[1]}, ${match[3]}${match[4]}${match[3]}, 1);`
                );
                callCount++;
            }
        }
        ...
        const generatedFile = path.join(config.temp.root, 'twig-terms', 'twig-terms.php');
        fs.mkdirSync(path.dirname(generatedFile), { recursive: true });
        fs.writeFileSync(generatedFile, lines.join('\n') + '\n', 'utf8');
        this.log('🧵', `Collected ${callCount} term(s) from ${twigFiles.length} Twig template(s)`);
    ...
    extractPHPTerms() {
        ...
        try {
            ...
            // Twig templates (the Member Portal's theme templates) go through
            // the same xgettext pass, via a generated PHP file.
            const twigTermsFile = this.extractTwigTerms();
            if (twigTermsFile) {
                phpFiles.push(twigTermsFile);
            }
