...
async function loadLocaleFiles(localeConfig) {
    ...
    try {
        ...
        if (localeConfig.locale) {
            const i18nPath = `${rootPath}/locale/JSONKeys/${localeConfig.locale}.json`;
            try {
        ...
        if (localeConfig.momentLocale && typeof moment !== 'undefined') {
            const momentPath = `${rootPath}/skin/external/locales/moment/${localeConfig.momentLocale}.js`;
            promises.push(
        ...
        if (localeConfig.datePicker) {
            const dpPath = `${rootPath}/skin/external/locales/bootstrap-datepicker/bootstrap-datepicker.${localeConfig.languageCode}.min.js`;
            promises.push(
                loadScript(dpPath)
        ...
        if (localeConfig.select2) {
            const s2Path = `${rootPath}/skin/external/locales/select2/${localeConfig.languageCode}.js`;
            promises.push(
                loadScript(s2Path)
